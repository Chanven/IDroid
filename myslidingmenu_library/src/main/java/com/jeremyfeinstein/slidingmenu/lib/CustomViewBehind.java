...
public class CustomViewBehind extends ViewGroup {
	...
	private static final int INTERCEPT_THRESHOLD = 5; //dips
	...
	public CustomViewBehind(Context context, AttributeSet attrs) {
		...
		mInterceptThreshold = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 
				INTERCEPT_THRESHOLD, getResources().getDisplayMetrics());
	}
	...
	public void setMarginThreshold(int marginThreshold) {
		mMarginThreshold = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 
				marginThreshold, getResources().getDisplayMetrics());
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		boolean handled = !mChildrenEnabled;
		int action = e.getAction();
		switch (action) {
		case MotionEvent.ACTION_MOVE:
			float dx = e.getX() - mLastMotionX;
			if(thisSlideAllowed(dx)){
				handled = true;
			}
			break;
	...
	private boolean thisSlideAllowed(float dx) {
		...
		if(xDiff >= mInterceptThreshold){
			allowed = true;
		}
		return allowed;
	}
	/**
	 * set threshold,the sliding distance(X) less than the threshold will not be intercepted.
	 * @param threshold units by dips.
	 */
	public void setInterceptThreshold(int threshold){
		mInterceptThreshold = threshold;
	}
	/**
	 * 
	 * @return the value in pixs,which is set through {@link CustomViewBehind#setInterceptThreshold(int)}
	 */
	public int getInterceptThreshold(){
		return mInterceptThreshold;
	}
